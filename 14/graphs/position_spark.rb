...
teams.each_with_index { |team, i|
    ...
    if i%2==0 then
    svg << SVG::Rect.new(wscale, y-10, wscale*(legend-2), height) { self.style = SVG::Style.new(:fill => '#ffdddd')}
    else
    svg << SVG::Rect.new(wscale, y-10, wscale*(legend-2), height) { self.style = SVG::Style.new(:fill => '#ddffdd')}
    end
